...
func getInstanceCountLimit(info *projectInfo, instanceType instancetype.Type) (int, int, error) {
	...
	for _, inst := range info.Instances {
		if inst.Instance.Type == instanceType {
			instanceCount++
		}
	}
...
func checkRestrictions(project api.Project, instances []instanceInfo, profiles []api.Profile) error {
	...
	for _, info := range instances {
		err := entityConfigChecker(info.Instance.Type, info.Instance.Name, info.Config)
		if err != nil {
			return err
		}

		err = entityDevicesChecker(info.Instance.Type, info.Instance.Name, info.Devices)
		if err != nil {
			return err
		}
	}
...
func AllowInstanceUpdate(tx *db.ClusterTx, projectName, instanceName string, req api.InstancePut, currentConfig map[string]string) error {
	var updatedInstance *instanceInfo
	info, err := fetchProject(tx, projectName, true)
	if err != nil {
		return err
	}
	...
	for i, instanceInfo := range info.Instances {
		if instanceInfo.Instance.Name != instanceName {
			continue
		}
	...
	err = checkRestrictionsOnVolatileConfig(
		info.Project, updatedInstance.Instance.Type, updatedInstance.Instance.Name, req.Config, currentConfig, false)
	if err != nil {
		return err
	}
...
func validateTotalInstanceCountLimit(instances []instanceInfo, value, project string) error {
	if value == "" {
		return nil
	}
...
// the current count.
func validateInstanceCountLimit(instances []instanceInfo, key, value, project string) error {
	if value == "" {
		return nil
	}
...
func fetchProject(tx *db.ClusterTx, projectName string, skipIfNoLimits bool) (*projectInfo, error) {
	...
	instances := make([]instanceInfo, len(dbInstances))
	for i, inst := range dbInstances {
		...
		instances[i] = instanceInfo{
			Instance: inst,
			Config:   config,
			Devices:  db.DevicesToAPI(devices),
		}
	}
...
func expandInstancesConfigAndDevices(instances []instanceInfo, profiles []api.Profile) []instanceInfo {
	expandedInstances := make([]instanceInfo, len(instances))
...
func getInstanceLimits(instance instanceInfo, project api.Project, keys []string, skipUnset bool) (map[string]int64, error) {
	var err error
	...
	for _, key := range keys {
		...
		if key == "limits.disk" {
			...
			if err != nil {
				return nil, fmt.Errorf("Failed getting root disk device for instance %q in project %q: %w", instance.Instance.Name, instance.Instance.Project, err)
			}
			...
			if !ok || value == "" {
				...
				return nil, fmt.Errorf(`Instance %q in project %q has no "size" config set on the root device either directly or via a profile`, instance.Instance.Name, instance.Instance.Project)
			}
			...
			if err != nil {
				...
				return nil, fmt.Errorf("Failed parsing %q for instance %q in project %q", key, instance.Instance.Name, instance.Instance.Project)
			}

			// Add size.state accounting for VM root disks.
			if instance.Instance.Type == instancetype.VM {
				...
				if err != nil {
					...
					return nil, fmt.Errorf("Failed parsing %q for instance %q in project %q", "size.state", instance.Instance.Name, instance.Instance.Project)
				}
			...
			if !ok || value == "" {
				...
				return nil, fmt.Errorf("Instance %q in project %s has no %q config, either directly or via a profile", instance.Instance.Name, instance.Instance.Project, key)
			}
