...
func (m *Method) getRefs(buf *file.Buffer, refMapping *Mapping) error {
	...
	refParentList := m.entity + lex.Camel(refList)
	...
	case ReferenceTable:
		...
		buf.L("for _, ref := range %s[entity.ID] {", refParentList)
	...
	case MapTable:
		buf.L("%s, err := c.Get%s(\"%s\")", refParentList, lex.Plural(refStruct), m.entity)
		m.ifErrNotNil(buf, "nil", "err")
		buf.L("%s, ok := %s[entity.ID]", refList, refParentList)
		buf.L("if !ok {")
...
func (m *Method) createRefs(buf *file.Buffer, refMapping *Mapping) error {
	...
	switch refMapping.Type {
	case ReferenceTable:
		buf.L("%s.ReferenceID = int(%sID)", refVar, m.entity)
	...
	case MapTable:
		buf.L("referenceID := int(%sID)", m.entity)
...
func (m *Method) updateRefs(buf *file.Buffer, refMapping *Mapping) error {
	...
	buf.L("err := c.Update%s(\"%s\", int(%sID), %s)", lex.Plural(refStruct), m.entity, m.entity, refList)
	...
	case EntityTable:
		...
		case "GetMany":
			if m.ref == "" {
				...
				args = fmt.Sprintf("entity %s", mapping.Name)
		...
		case "Create":
			if m.ref == "" {
				...
				switch refMapping.Type {
				case ReferenceTable:
					args = fmt.Sprintf("%sID int64, %s %s", m.entity, lex.Minuscule(m.ref), m.ref)
				case MapTable:
					args = fmt.Sprintf("%sID int64, %s map[string]string", m.entity, lex.Minuscule(m.ref))
				}
			}
		case "CreateOrReplace":
			entityCreate, ok := m.config["struct"]
