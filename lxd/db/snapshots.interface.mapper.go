...
type InstanceSnapshotGenerated interface {
	...
	GetInstanceSnapshotDevices(entity InstanceSnapshot) (map[string]Device, error)

	// GetInstanceSnapshotConfig returns all available InstanceSnapshot Config
	// generator: instance_snapshot GetMany
	GetInstanceSnapshotConfig(entity InstanceSnapshot) (map[string]string, error)
	...
	CreateInstanceSnapshotDevice(instance_snapshotID int64, device Device) error

	// CreateInstanceSnapshotConfig adds a new instance_snapshot Config to the database.
	// generator: instance_snapshot Create
	CreateInstanceSnapshotConfig(instance_snapshotID int64, config map[string]string) error
