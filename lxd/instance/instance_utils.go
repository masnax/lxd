...
func CreateInternal(s *state.State, args db.InstanceArgs, clearLogDir bool) (Instance, *operationlock.InstanceOperation, revert.Hook, error) {
	...
	err = s.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		clusterDevices, err := cluster.APIToDevices(args.Devices.CloneNative())
		if err != nil {
			return err
		}
		...
		if args.Snapshot {
			...
			newArgs, err := s.ToInstanceArgs(ctx, tx.Tx(), instance)
			if err != nil {
				return err
			}
		...
		err = cluster.CreateInstanceDevices(ctx, tx.Tx(), instanceID, clusterDevices)
		if err != nil {
			return err
		}
