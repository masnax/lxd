...
func LoadByProjectAndName(s *state.State, project, name string) (Instance, error) {
	...
	err := s.Cluster.Transaction(func(tx *db.ClusterTx) error {
		if strings.Contains(name, shared.SnapshotDelimiter) {
			...
			if err != nil {
				return fmt.Errorf("Failed to fetch instance %q in project %q: %w", name, project, err)
			}

			snapshot, err := tx.GetInstanceSnapshot(project, instanceName, snapshotName)
			if err != nil {
				return fmt.Errorf("Failed to fetch snapshot %q of instance %q in project %q: %w", snapshotName, instanceName, project, err)
			}
