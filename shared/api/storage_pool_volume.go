...
func (volume *StorageVolume) URL(apiVersion string, pool string, project string, target string) *URL {
	volPath := fmt.Sprintf("%s/volumes/%s/%s", pool, volume.Type, volume.Name)
